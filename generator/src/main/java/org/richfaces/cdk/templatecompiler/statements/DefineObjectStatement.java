...
public class DefineObjectStatement extends FreeMarkerTemplateStatementBase {
    ...
    public void setObject(String name, String type, String initializationExpression, boolean cast) {
        try {
            this.name = name;
            this.cast = cast;
            ELType expectedType = Strings.isEmpty(type) ? TypesFactory.OBJECT_TYPE : typesFactory.getType(type);
            if (!Strings.isEmpty(initializationExpression)) {
                initializationStatement = parser.parse(initializationExpression, this, expectedType);
                initializationStatement.setParent(this);
                this.type = Strings.isEmpty(type) ? initializationStatement.getType() : expectedType;
            } else {
